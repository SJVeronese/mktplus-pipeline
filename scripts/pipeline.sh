...
run_crosscal() {
    ...
    done < <(singularity exec "${BINDS[@]}" "${CASA_CONTAINER}" "${cmd[@]}" 2>&1)
...
run_inspect() {
    ...
    for plot in "${plot_list[@]}"; do
        ...
        done < <(singularity exec "${BINDS[@]}" "${SHADEMS_CONTAINER}" shadems "${input}" "${cmd[@]}" 2>&1)
    done
...
run_listobs() {
    ...
    singularity exec "${BINDS[@]}" "${CASA_CONTAINER}" \
...
run_wsclean() {
    ...
    if [[ "${estimate,,}" == "true" ]]; then
        ...
        PARAMS=$(singularity exec "${BINDS[@]}" "${CASA_CONTAINER}" "${evaluate_cmd[@]}")
...
# define the container-internal mount points.
# These are the paths that scripts see inside the container
...
readonly CASA_CONTAINER="${CONT_PATH}/${CASA_IMG}"
readonly SHADEMS_CONTAINER="${CONT_PATH}/${SHADEMS_IMG}"
